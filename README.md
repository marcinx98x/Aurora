...
- Queue with **shuffle**, **repeat one/all** and **drag-to-reorder**. Continuous playthrough: when a track ends, the next one **loads and starts automatically** (app open or minimized — no extra Play tap). Order is sequential, or random when shuffle is on — failed loads stay on the current track (retry / Play), they never auto-skip ahead. The Dart queue is the source of truth; end-of-track is detected even when the native player skips `completed`. Remote songs use a **single `AudioSource.uri`** and **one HTTP stream at a time**, so the resolver is not blocked by a second `/stream` while the first is still playing. Transient stream failures **retry** the same track; the next remote track is **warmed** in the background so advance usually hits a cache hit.
