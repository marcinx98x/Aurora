...
class PlayerController extends Notifier<PlayerState> {
  ...
  bool get _isRemotePlayback {
    try {
      return ref.read(devicesControllerProvider).isRemote;
    } catch (_) {
      return false;
    }
  }
  ...
  Future<void> toggle() async {
    if (_isRemotePlayback) {
      ...
      if (state.isPlaying) {
        await devices.remotePause();
        state = state.copyWith(isPlaying: false);
      } else {
        await devices.remotePlay();
  ...
  Future<void> pause() async {
    if (_isRemotePlayback) {
      await ref.read(devicesControllerProvider.notifier).remotePause();
      state = state.copyWith(isPlaying: false);
  ...
  Future<void> seek(double fraction) async {
    ...
    if (_isRemotePlayback) {
      await ref.read(devicesControllerProvider.notifier).remoteSeek(target);
  ...
  Future<void> _loadCurrent({
    ...
    try {
      ...
      if (_isRemotePlayback) {
        ...
        await ref.read(devicesControllerProvider.notifier).remoteLoadTrack(
              track,
              uri,
              position: startAt,
  ...
  Future<void> setVolume(double v) async {
    ...
    _cancelFade();
    if (_isRemotePlayback) {
      await ref.read(devicesControllerProvider.notifier).remoteSetVolume(vol);
    } else {
